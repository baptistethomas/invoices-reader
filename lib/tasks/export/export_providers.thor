...
class ExportProviders < Thor
  ...
  def process
    ...
    headers = ["total kg", "categorized kg", "uncategorized kg", "with origin kg", "without origin kg", "with bio kg", "without bio kg", "eqco2 kg total"]
    ...
    providers.each do | provider |
      ...
      invoices = Invoice.get_invoices_from_provider(provider['id'])
      ...
      if invoices && invoices.count > 0
        invoices.each do |invoice|
          ...
          invoice_items = InvoiceItem.get_invoice_items_from_invoice(invoice['id'])
          if invoice_items && invoice_items.count > 0
            invoice_items.each do | invoice_item|
              ...
              unless invoice_item['kilogram'].nil?
                # Categorized
      ...
      provider_data_ = [total_kg, categorized_kg, uncategorized_kg, with_origin_kg, without_origin_kg, with_bio_kg, without_bio_kg, eqco2_total]
